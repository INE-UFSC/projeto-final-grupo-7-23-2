...
class MenuUI(UI):
    def __init__(self, context):
        ...
        self.buttons = [
               TextButton(pygame.font.Font(C().get_font('Pixeltype.ttf'),60).render('Jogar',True,'black'),'green',C().get_width()/2,C().get_height()/4,125,55, lambda: self.get_ctx().change_state(level_select_state.LevelSelectState(context))),
               TextButton(pygame.font.Font(C().get_font('Pixeltype.ttf'),60).render('Como Jogar',True,'black'),'cyan',C().get_width()/2,1.75*C().get_height()/4,230,55, lambda: self.get_ctx().change_state(instructions_state.InstructionsState(context))),
               TextButton(pygame.font.Font(C().get_font('Pixeltype.ttf'),60).render('Sair',True,'black'),'red',C().get_width()/2,2.5*C().get_height()/4,105,55, lambda: self.get_ctx().exit_game() )
        ]
